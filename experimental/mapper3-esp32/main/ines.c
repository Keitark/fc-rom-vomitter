                   ...
                   nescart_image_t *output, char *error, size_t error_length)
{
    ...
    if (mapper != NESCART_MAPPER_NROM && mapper != NESCART_MAPPER_CNROM) {
        set_error(error, error_length,
                  "mapper %u is not supported; this experiment accepts mapper 0 or 3",
                  mapper);
        return -1;
    }
    ...
    if ((mapper == NESCART_MAPPER_NROM && chr_banks != 1) ||
        (mapper == NESCART_MAPPER_CNROM && chr_banks != 2)) {
        set_error(error, error_length,
                  "mapper %u requires %u KiB CHR in this experiment, got %u banks",
                  mapper,
                  mapper == NESCART_MAPPER_CNROM ? 16u : 8u,
                  chr_banks);
        return -1;
    }
    if ((flags6 & 0x08u) != 0) {
    ...
    const size_t chr_size = (size_t)chr_banks * NESCART_CHR_BANK_SIZE;
    ...
    memcpy(output->data + NESCART_PRG_SIZE, chr, chr_size);
    if (chr_banks == 1) {
        memcpy(output->data + NESCART_PRG_SIZE + NESCART_CHR_BANK_SIZE,
               chr, NESCART_CHR_BANK_SIZE);
    }
    ...
    output->mapper = (nescart_mapper_t)mapper;
    output->chr_banks = chr_banks;
