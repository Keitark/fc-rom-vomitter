...
static controller_status_t s_status = {
    ...
    .message = "No image stored. Upload a mapper-0 or two-bank mapper-3 .nes file.",
};
...
static esp_err_t load_current_image(bool console_present)
{
    if (s_image == NULL) {
        ...
        set_mode(CONTROLLER_NO_IMAGE,
                 "No image stored. Upload a mapper-0 or two-bank mapper-3 .nes file.");
