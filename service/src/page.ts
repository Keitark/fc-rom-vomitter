...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .hero { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,.94fr); align-items:center; gap:clamp(24px,5vw,68px); padding:58px 0 45px; }
    ...
    h1 { margin-bottom:22px; font-size:clamp(2.85rem,6vw,5.4rem); line-height:1.09; letter-spacing:-.065em; font-weight:850; }
    .lead { max-width:37ch; margin-bottom:24px; color:#344b3f; font-size:clamp(1.08rem,1.6vw,1.3rem); line-height:1.75; }
    ...
    .board-frame { position:relative; margin:0; padding:20px; border:1px solid #d7d4e4; border-radius:26px; background:#dfdeef; box-shadow:0 20px 48px #1c3c3119; }
    .board-frame img { display:block; width:100%; height:auto; border-radius:12px; }
    .board-frame figcaption { color:#3d4650; padding:10px 5px 2px; font-size:.78rem; }
    .notice { display:flex; gap:14px; align-items:flex-start; margin:0 0 48px; padding:17px 19px; border:1px solid #e9d391; border-radius:16px; background:#fff8dc; color:#624e1e; }
    ...
    .work-area { display:grid; grid-template-columns:minmax(0,1.22fr) minmax(280px,.78fr); gap:20px; align-items:start; padding-bottom:70px; }
    ...
    @media (max-width:800px) { .hero { grid-template-columns:1fr; padding:32px 0; } .board-frame { max-width:620px; } .work-area { grid-template-columns:1fr; } .help-card { order:2; } }
    @media (max-width:600px) { .shell { width:min(100% - 28px,1160px); } .topbar { padding:14px 0; } .top-actions { gap:8px; } .about-link { display:none; } .brand { font-size:.95rem; } .brand-mark { width:38px; height:38px; } .language button { padding:5px 9px; } h1 { font-size:clamp(2.6rem,10vw,4rem); } .notice { display:block; } .notice strong { display:block; margin-bottom:5px; } .section-head { display:block; } .section-head p { margin-top:5px; } .steps { grid-template-columns:1fr; gap:10px; margin-bottom:38px; } .step { min-height:0; padding:17px; } .step-number { display:inline-grid; margin:0 10px 0 0; vertical-align:middle; } .step h3 { display:inline; } .step p { margin-top:6px; } .board-frame { padding:12px; } .file-picker { align-items:flex-start; flex-direction:column; } }
...
<body>
  ...
  <main class="shell">
    <section class="hero" aria-labelledby="page-title">
      ...
      <figure class="board-frame"><img src="https://raw.githubusercontent.com/Keitark/fc-rom-vomitter/main/docs/images/board-top.png" width="1992" height="1488" alt="実際のWi-Fiファミコンカートリッジ基板の3Dレンダリング" data-i18n-alt="boardAlt"><figcaption data-i18n="boardCaption">展示用Wi-Fiカートリッジの基板レンダリング（実際の設計データ）</figcaption></figure>
    </section>
    ...
    <section id="how" aria-labelledby="how-title"><div class="section-head"><h2 id="how-title" data-i18n="howTitle">使い方は3ステップ</h2><p data-i18n="howSub">まずは対応ファイルを1つご用意ください。</p></div><div class="steps">
      <article class="step"><span class="step-number" aria-hidden="true">01</span><h3 data-i18n="step1Title">ゲームを選ぶ</h3><p data-i18n="step1Body">自作、または送信の許可がある .nes ファイルを選びます。</p></article>
      <article class="step"><span class="step-number" aria-hidden="true">02</span><h3 data-i18n="step2Title">形式を確認</h3><p data-i18n="step2Body">このカートリッジに合う形式か、ページが自動で調べます。</p></article>
      <article class="step"><span class="step-number" aria-hidden="true">03</span><h3 data-i18n="step3Title">受付状況を見る</h3><p data-i18n="step3Body">登録後はこのページで待ち順や結果を確認できます。</p></article>
    </div></section>
  ...
  <script>
    const copy = {
      ...
      en: {
        ...
        title:"Famicom Game Drop | Share a homebrew game",navHow:"How it works",eyebrow:"A real Famicom exhibition project",heroTitle:"Your game, on a Famicom.",heroLead:"Choose a homebrew .nes file. We check its format and add it to the exhibition cartridge's queue.",point1:"Works on your phone",point2:"No account needed",point3:"Temporary storage",boardAlt:"3D render of the actual Wi-Fi Famicom cartridge board",boardCaption:"Wi-Fi exhibition cartridge, rendered from the real board design",noticeTitle:"Current demo status",noticeBody:"You can try file validation and the queue today. Automatic pickup by the cartridge is still being built, so uploading a file does not yet change the game on the console.",howTitle:"Three simple steps",howSub:"Have one compatible game file ready.",step1Title:"Choose a game",step1Body:"Select a .nes file you made or have permission to share.",step2Title:"We check the format",step2Body:"The page checks whether the file works with this cartridge.",step3Title:"Follow its progress",step3Body:"After submitting, see its place in the queue and the result here.",uploadTitle:"Submit a game file",uploadIntro:"Choose your file and confirm you are allowed to send it.",fileLabel:"Game file (.nes)",fileHint:"See “Which files work?” for the supported format.",rights:"I have permission to use and send this file.",submit:"Check file and join queue",jobTitle:"Your submission",bookmarkHint:"Save this page's URL to check the result later.",copyLink:"Copy status link",copied:"Status link copied.",copyFailed:"Could not copy. Please save the URL from your browser.",helpTitle:"Before you submit",compatibleTitle:"Which files work?",compatibleBody:"The current public version accepts some homebrew Famicom games in .nes files. Do not upload commercial game copies here.",afterTitle:"What happens next?",afterBody:"We check the file and place compatible games in a queue. Automatic cartridge pickup is still in development.",privacyTitle:"Is my file kept?",privacyBody:"Your file is stored privately and deleted after about one hour. No sign-in is needed.",technicalTitle:"Show exact file requirements",technicalBody:"iNES format, Mapper 0 (NROM), 16 or 32 KiB PRG, and 8 KiB CHR. CHR-RAM, NES 2.0, and four-screen mirroring are not supported.",footerLine:"Open hardware experiment · Keitark",sourceLink:"View the design and source",licenseLink:"Image license",validating:"Checking your file…",loading:"Checking your submission…",networkError:"Connection lost. We will try again in a few seconds.",errorGeneric:"We could not submit the file. Please try another file.",position:"Queue position: ",until:"Submission expires: ",states:{queued:["In the queue","Your compatible file has been accepted. Automatic cartridge pickup is still in development."],claimed:["Cartridge is checking","The cartridge has started receiving the file."],downloaded:["Transferring","The cartridge has received the file."],deferred:["Waiting for a safe moment","Waiting until the console can safely switch games."],installed:["Ready on the cartridge","The cartridge has installed the game. Follow the exhibition staff's instructions to use the console."],unchanged:["Already active","This game is already active on the cartridge."],failed:["Could not install","The cartridge could not process the file. Please ask the exhibition staff."],expired:["Time expired","The file was not processed in time and has been deleted."],cancelled:["Submission cancelled","The exhibition staff cancelled this submission."]},errors:{authorization_required:"Please confirm you have permission to send this file.",missing_file:"Choose a .nes file.",too_large:"This file is too large.",short_header:"We could not read this file.",bad_magic:"This is not a valid .nes file.",nes2_unsupported:"This file format is not supported yet (NES 2.0).",mapper_unsupported:"This game's mapping is not supported yet (Mapper 0 only).",prg_geometry:"The game data is outside the supported size.",chr_geometry:"The graphics data is outside the supported size (8 KiB CHR required).",four_screen_unsupported:"This screen mode is not supported yet.",length_mismatch:"The file length does not match its header.",duplicate:"This file has already been submitted.",cooldown:"Please wait a moment before submitting again.",queue_full:"The queue is full. Please try later.",upload_invalid:"We could not read the file.",not_found:"We could not find this submission link."}
      }
    };
    ...
    let lang="ja",timer=null,currentUrl="",lastJob=null;
    ...
    function renderJob(job){lastJob=job;panel.hidden=false;const state=copy[lang].states[job.state]||[job.state,t("errorGeneric")];stateEl.textContent=state[0];description.textContent=state[1];const parts=[];if(Number.isInteger(job.queue_position))parts.push(t("position")+job.queue_position);if(job.expires_at){const date=new Date(job.expires_at);if(!Number.isNaN(date.getTime()))parts.push(t("until")+new Intl.DateTimeFormat(lang==="ja"?"ja-JP":"en-US",{hour:"2-digit",minute:"2-digit"}).format(date));}extra.textContent=parts.join(" · ");}
